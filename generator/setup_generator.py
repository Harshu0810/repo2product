...
#  Target: Linux/macOS, CPU-only, {self.ram_gb}GB RAM
# ============================================================
...
REM  Target: Windows, CPU-only, {self.ram_gb}GB RAM
REM ============================================================
    ...
    def _generate_run_sh(self) -> str:
        ...
        if not self.constraints.get("has_gpu", False):
            env_export += [
                ...
                "export OMP_NUM_THREADS=4         # Optimize OpenMP thread count",
...
"""
    ...
    def _generate_env_template(self) -> str:
        ...
        # Add CPU-specific vars
        lines += [
            "# ── CPU/Performance (auto-set in run.sh) ─────────",
            "CUDA_VISIBLE_DEVICES=  # Empty = disable GPU",
            "OMP_NUM_THREADS=4",
...
| Target System | CPU-only, {self.ram_gb}GB RAM, {self.os} |
...
- **CPU**: Intel 7th Gen — expect slower ML inference vs GPU
- **RAM**: {self.ram_gb}GB — monitor with `watch -n2 free -h`
- **Thread count**: Set `OMP_NUM_THREADS=4` for 4-core optimization
